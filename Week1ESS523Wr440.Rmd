...
```{bash}
git clone {https://github.com/hungryhydrologist/Week-1-Intro.git}
```
...
1.  Why don't the following lines of code work? Tweak each one so the code runs
    ...
    myList[["Fruit"]]
    ```
You need two brackets to index lists. 
    ```{r}
...
"Length" is misspelled. 
    ...
    ```
You need to put the data frame with a $ inside of the brackets. That tells R which data frame to look in, then island is the column you want to index.  
...
# Three species of penguin are in the penguins dataset.
unique(penguins$island)
# Biscoe, Dream, and Torgersen are the islands in this dataset. 
```
...
```{r}
penguins_3col <- penguins[c('species','island','flipper_length_mm')]
PenguinsonDream <- penguins_3col[penguins_3col$island == 'Dream',]
```
...
```{r}
AdeliePenguins <- penguins_3col[penguins_3col$species=='Adelie',]
  DreamAdeliePenguins <- AdeliePenguins[AdeliePenguins$island=='Dream',]

mean(DreamAdeliePenguins$flipper_length_mm, na.rm = T)
```
...
meanbodymass <- mean(penguins$body_mass_g, na.rm = T)
mutate(penguins, size_group = if_else(body_mass_g > meanbodymass, paste0('large'), paste0('small')))
...
```{r}
big_penguins_year <- penguins %>%
  group_by(year, size_group, species) %>%
...
```{r}
PenguinsonDream2 <- penguins[penguins$island == 'Dream',]
ggplot(PenguinsonDream2)+ 
...
```
# raster data - work with elevation package
```{r}
elevation_data <- get_elev_raster(counties, z = 7)
qtm(elevation_data)
```
```{r}
...
```
```{r}
# work with terra package. Convert raster layer to terra raster object (spat raster object)
elevation_data <- rast(elevation_data)
names(elevation_data) <- "Elevation"
#crop to Larimer county
st_crs(counties)
crs(counties) == crs(elevation_data)
# project raster to CRS of counties
elevation_prj <- terra::project(elevation_data, counties)
#crop eelvation to counties extent
elevation_crop <- crop(elevation_data, ext(counties))
...
#save .RData to save your objects and stuff cause that's a smaller file size 
save(counties, roads, file = 'data/spatialobjects.RData')
load('data/spatialobjects.RData')
```
